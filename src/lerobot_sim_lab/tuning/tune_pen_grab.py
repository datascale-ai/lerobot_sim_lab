...
def print_help():
    ...
    print("  save! <name> [steps]           - 强制覆盖同名关键帧，可选指定插值步数")
    ...
    print("  2. 调整关节到接近pen4的位置")
    print("  3. save 1-approach 50 - 保存第一个姿态（自动在前面添加 0-home 帧）")
    print("  4. load 1-approach    - 重新加载刚保存的姿态（可选）")
    print("  5. 调整到抓取位置")
    print("  6. save 2-grasp 40    - 保存抓取姿态（40步插值）")
    print("  7. 关闭夹爪: 5, set 0.3")
    print("  8. save 3-lift 35     - 保存提起姿态（35步插值）")
    print("  9. 调整到纸盒上方")
    print("  10. save 4-place 30   - 保存放置姿态（30步插值）")
    print("  11. 打开夹爪: 5, set -0.15")
    print("  12. save 5-release 15 - 保存松开姿态（15步插值，快速）")
    print("  13. waypoints         - 查看所有关键帧（会看到自动添加的 0-home）")
    
    print("\n📹 回放和录制视频:")
    print("  在另一个终端运行:")
    print("    python -m lerobot_sim_lab.tuning.tune_pen_grab --mode playback")
    print("    python -m lerobot_sim_lab.tuning.tune_pen_grab --mode playback --record")
    print("    python -m lerobot_sim_lab.tuning.tune_pen_grab --mode playback --waypoints 2 4")
    print("="*80)
    print()
...
def main():
    ...
    if not config_file.exists():
        print(f"\n❌ 错误: {config_file} 不存在")
        print("请先在另一个终端运行:")
        print("  python -m lerobot_sim_lab.tuning.tune_pen_grab --mode live")
        return
    ...
    while True:
        try:
            ...
            if cmd in ['quit', 'q']:
                ...
                # 解析 insert 命令
                # 格式: insert <name> after/before <ref_name> [steps]
                try:
                    parts = cmd.split()
                    ...
                    print("❌ 步数必须是整数")
                    print("   用法: insert <name> after/before <ref_name> [steps]")
...
if __name__ == "__main__":
    ...
    if mode == "live":
        from lerobot_sim_lab.tuning._tune_pen_grab_live import main as live_main

        live_main()
    elif mode == "playback":
        from lerobot_sim_lab.tuning._tune_pen_grab_playback import main as playback_main
